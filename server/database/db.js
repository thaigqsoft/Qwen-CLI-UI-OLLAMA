...
const userDb = {
  ...
  createUser: (username, passwordHash) => {
    try {
      const stmt = db.prepare('INSERT INTO qwencliui_users (username, password_hash) VALUES (?, ?)');
  ...
  getUserById: (userId) => {
    try {
      const row = db.prepare('SELECT id, username, created_at, last_login FROM qwencliui_users WHERE id = ? AND is_active = 1').get(userId);
      return row;
    } catch (err) {
      throw err;
    }
