...
import { X, Plus, Settings, Shield, AlertTriangle, Moon, Sun, Server, Edit3, Trash2, Play, Globe, Terminal, Zap, Volume2, RefreshCw, CheckCircle, XCircle, Cpu } from 'lucide-react';
...
function ToolsSettings({ isOpen, onClose }) {
  ...
  // Provider settings - 'qwen-cli' or 'ollama'
  const [provider, setProvider] = useState('qwen-cli');
  
  // Ollama settings
  const [ollamaUrl, setOllamaUrl] = useState('http://localhost:11434');
  const [ollamaModels, setOllamaModels] = useState([]);
  const [ollamaSelectedModel, setOllamaSelectedModel] = useState('');
  const [ollamaConnectionStatus, setOllamaConnectionStatus] = useState(null); // null, 'checking', 'connected', 'error'
  ...
  const [selectedModel, setSelectedModel] = useState('qwen3-coder-next:cloud');
  ...
  const availableModels = [
    { value: 'qwen3-coder-next:cloud', label: 'qwen3-coder-next:cloud', description: 'Advanced Qwen coding model' }
  ];
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (data.models && data.models.length > 0 && !ollamaSelectedModel) {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (savedSettings) {
        ...
        setSelectedModel(settings.selectedModel || 'qwen3-coder-next:cloud');
        setEnableNotificationSound(settings.enableNotificationSound || false);
        
        // Load provider and Ollama settings
      ...
      // If provider is Ollama, check connection and load models
      if (settings?.provider === 'ollama') {
        await handleOllamaRefresh();
      }
    } catch (error) {
      ...
      <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
        <div className="flex items-center justify-between">
          ...
          >
              ...
              <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <div className="font-medium text-foreground">Qwen CLI</div>
                      <div className="text-xs text-muted-foreground mt-1">Use Qwen CLI locally</div>
                    </button>
                    <button
                      onClick={() => {
                        setProvider('ollama');
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground mt-1">Use Ollama (local/remote)</div>
                    </button>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="px-3 py-1 text-sm bg-green-600 hover:bg-green-700 text-white rounded-lg flex items-center gap-2 disabled:opacity-50"
                      >
                    ...
                    <div>
                      ...
                      {ollamaModels.length > 0 ? (
                        <select
                          ...
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-lg focus:ring-green-500 focus:border-green-500"
                        >
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400 py-2">
                          {ollamaLoading ? 'Loading models...' : 'No models found. Make sure Ollama is running and has models installed.'}
                        </div>
                      )}
                    </div>
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 p-2 rounded">
                      <p>💡 Configure OLLAMA_URL in .env file to connect to a remote Ollama server.</p>
                      <p className="mt-1">Example: OLLAMA_URL=http://192.168.1.100:11434</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Qwen CLI Model Selection - Only show when provider is qwen-cli */}
            {provider === 'qwen-cli' && (
            <div className="space-y-4">
              <div className="flex items-center gap-3">
