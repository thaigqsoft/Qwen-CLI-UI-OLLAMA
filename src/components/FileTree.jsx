...
import { Folder, FolderOpen, File, FileText, FileCode, List, TableProperties, Eye, RefreshCw } from 'lucide-react';
import { cn } from '../lib/utils';
import CodeEditor from './CodeEditor';
import ImageViewer from './ImageViewer';
import { api } from '../utils/api';

function FileTree({ selectedProject, isActive }) {
  ...
  const [lastActiveState, setLastActiveState] = useState(false);

  useEffect(() => {
    // Fetch files when tab becomes active or project changes
    if (selectedProject && isActive) {
      // Always fetch when transitioning from inactive to active
      if (!lastActiveState || selectedProject) {
        fetchFiles();
      }
    }
    setLastActiveState(isActive);
  }, [selectedProject, isActive, lastActiveState]);
